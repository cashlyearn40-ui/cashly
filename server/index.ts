...
function setupSecurity(app: express.Application) {
  // Security headers (CSP disabled — Expo web uses inline scripts)
  app.use(helmet({
  ...
  // Rate limit: financial mutations — 30 per minute per IP
  ...
  app.use("/api/user/earn", financeLimiter);
  app.use("/api/user/withdraw", financeLimiter);
}
...
function setupCors(app: express.Application) {
  app.use((req, res, next) => {
    ...
    // Allow any Replit-hosted subdomain (covers *.expo.worf.replit.dev, etc.)
    ...
    if (origin && (origins.has(origin) || isLocalhost || isReplitOrigin)) {
      ...
      res.header(
        ...
        "Content-Type, x-admin-token, x-user-token",
      );
...
function setupRequestLogging(app: express.Application) {
  app.use((req, res, next) => {
    ...
    res.on("finish", () => {
      ...
      // Never log auth/admin responses — they may contain tokens or password status
